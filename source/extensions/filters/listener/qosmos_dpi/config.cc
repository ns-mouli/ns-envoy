...
public:
  Network::ListenerFilterFactoryCb createListenerFilterFactoryFromProto(
    ...
    // iff proto_config.verdict_cache_correction_enabled AND the derived
    // per-worker cap is > 0. Only the FIRST listener config to trigger
    // engine construction actually decides whether the cache is on —
    // subsequent configs (same singleton) inherit that shape. This
    // matches how the listener filter has always treated the singleton
    // engine.
    //
    // Sizing:
    //   proto.verdict_cache_total_entries == 0  ⇒ per-worker = 100000
    //     (the built-in default deliberately skips the divide-by-workers
    //     step so out-of-the-box deployments get a generous cache).
    ...
    if (proto_config.verdict_cache_correction_enabled()) {
      if (proto_config.verdict_cache_total_entries() == 0U) {
        cache_max_entries = 100000U;
      } else {
